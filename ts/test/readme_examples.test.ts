...
//     (\`new PollinationsAiSDK(...)\` / \`PollinationsAiSDK.test(...)\` become
//     \`PollinationsAiSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'PollinationsAiSDK'
...
const TEST_SEED = {"entity":{"generate_text":{"test01":{"id":"test01"}},"image_generation":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PollinationsAiSDK(...)\` and any
// \`PollinationsAiSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PollinationsAiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PollinationsAiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
