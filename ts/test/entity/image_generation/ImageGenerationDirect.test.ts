...
describe('ImageGenerationDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new PollinationsAiSDK({
  ...
  test('direct-load-image_generation', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-image_generation', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['POLLINATIONS_AI_TEST_IMAGE_GENERATION_ENTID']
