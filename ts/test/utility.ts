...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.POLLINATIONS_AI_TEST_LIVE ||
    'TRUE' === process.env.POLLINATIONS_AI_TEST_OVERRIDE
  ) {
  ...
  m.POLLINATIONS_AI_TEST_EXPLAIN = process.env.POLLINATIONS_AI_TEST_EXPLAIN || m.POLLINATIONS_AI_TEST_EXPLAIN
