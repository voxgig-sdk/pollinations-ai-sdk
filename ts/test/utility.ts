...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.POLLINATIONSAI_TEST_LIVE ||
    'TRUE' === process.env.POLLINATIONSAI_TEST_OVERRIDE
  ) {
  ...
  m.POLLINATIONSAI_TEST_EXPLAIN = process.env.POLLINATIONSAI_TEST_EXPLAIN || m.POLLINATIONSAI_TEST_EXPLAIN
