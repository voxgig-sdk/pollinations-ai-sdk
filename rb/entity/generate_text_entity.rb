...
class GenerateTextEntity
  ...
  def make
    opts = @_entopts.dup
    GenerateTextEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PollinationsAiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GenerateText, Hash] the current GenerateText data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = PollinationsAiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GenerateText fields)
  def match_get
  ...
  #
  # @param reqdata [GenerateTextCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [GenerateText, Hash] the created GenerateText; raises PollinationsAiError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = PollinationsAiHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
