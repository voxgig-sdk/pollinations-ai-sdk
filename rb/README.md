# PollinationsAi Ruby SDK

The Ruby SDK for the PollinationsAi API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
# Create
created, _ = client.GenerateText(nil).create({ "name" => "Example" }, nil)
...
result, err = client.PollinationsAi(nil).load(
  { "id" => "test01" }, nil
)
...
```
rb/
├── PollinationsAi_sdk.rb       -- Main SDK module
