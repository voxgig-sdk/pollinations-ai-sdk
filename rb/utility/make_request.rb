...
module PollinationsAiUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PollinationsAiResponse.new({})
    ...
    if fetch_err
      ...
      response = PollinationsAiResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PollinationsAiResponse.new(fetched)
    else
