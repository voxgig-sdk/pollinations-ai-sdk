...
require_relative 'PollinationsAi_types'
...
class PollinationsAiSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = PollinationsAiHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PollinationsAiHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, PollinationsAiFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    PollinationsAiUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = PollinationsAiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = PollinationsAiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = PollinationsAiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue PollinationsAiError => err
    ...
    if fetched.is_a?(Hash)
      status = PollinationsAiHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PollinationsAiError.new(
        "graphql_error", "PollinationsAiSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = PollinationsAiSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
