...
// PollinationsAi SDK pipeline test
//
...
{
    private static function utility(): PollinationsAiUtility
    {
        return new PollinationsAiUtility();
    }
    ...
    private static function ctx(array $over = []): PollinationsAiContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new PollinationsAiContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new PollinationsAiOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof PollinationsAiError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new PollinationsAiOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = PollinationsAiMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new PollinationsAiOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = PollinationsAiMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new PollinationsAiOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = PollinationsAiMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PollinationsAiSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = PollinationsAiMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new PollinationsAiResponse([]),
            'result' => new PollinationsAiResult([]),
        ]);
        [, $err] = PollinationsAiMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new PollinationsAiSpec([]),
            'response' => null,
            'result' => new PollinationsAiResult([]),
        ]);
        [, $err] = PollinationsAiMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new PollinationsAiSpec(['step' => 's']),
            'response' => new PollinationsAiResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new PollinationsAiResult([]),
        ]);
        [, $err] = PollinationsAiMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new PollinationsAiSpec(['step' => 's']),
            'response' => new PollinationsAiResponse(self::resp(200, ['v' => 1])),
            'result' => new PollinationsAiResult([]),
        ]);
        [, $err] = PollinationsAiMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new PollinationsAiSpec(['step' => 's']),
            'response' => new PollinationsAiResponse(self::resp(200, ['v' => 2])),
            'result' => new PollinationsAiResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        PollinationsAiMakeResponse::call($ctx);
    ...
    {
        $preset = new PollinationsAiResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new PollinationsAiSpec([]),
            'response' => new PollinationsAiResponse([]),
            'result' => new PollinationsAiResult([]),
        ...
        [$got, $err] = PollinationsAiMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new PollinationsAiResult([])]);
        [, $err] = PollinationsAiMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new PollinationsAiSpec([]), 'result' => null]);
        [, $err] = PollinationsAiMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PollinationsAiSpec(['step' => 's']),
            'result' => new PollinationsAiResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new PollinationsAiOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PollinationsAiMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PollinationsAiSpec(['step' => 's']),
            'result' => new PollinationsAiResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new PollinationsAiOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PollinationsAiMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PollinationsAiResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new PollinationsAiSpec([]),
            'result' => new PollinationsAiResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = PollinationsAiMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PollinationsAiMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new PollinationsAiError('boom', 'boom');
        $utility->fetcher = function (PollinationsAiContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PollinationsAiSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = PollinationsAiMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (PollinationsAiContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = PollinationsAiMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(PollinationsAiResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        PollinationsAiMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (PollinationsAiContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PollinationsAiSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = PollinationsAiMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new PollinationsAiResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new PollinationsAiSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = PollinationsAiMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = PollinationsAiMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = PollinationsAiMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new PollinationsAiResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, PollinationsAiDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new PollinationsAiResult(['ok' => false])]);
        $this->expectException(PollinationsAiError::class);
        PollinationsAiDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new PollinationsAiResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', PollinationsAiMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        PollinationsAiMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new PollinationsAiBaseFeature();
        $b = new PollinationsAiBaseFeature();
        PollinationsAiFeatureAdd::call($ctx, $a);
        PollinationsAiFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): PollinationsAiBaseFeature
    {
        $f = new PollinationsAiBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        PollinationsAiFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): PollinationsAiContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new PollinationsAiSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PollinationsAiPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        PollinationsAiPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new PollinationsAiResponse(['headers' => null]),
            'result' => new PollinationsAiResult([]),
        ]);
        PollinationsAiResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        PollinationsAiResultBody::call($ctx);
