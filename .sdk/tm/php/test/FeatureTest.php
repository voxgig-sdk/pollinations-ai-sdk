...
// PollinationsAi SDK feature test
//
...
class FtCtrl extends PollinationsAiControl
{
...
class FtClient
{
    ...
    private PollinationsAiUtility $utility;

    public function __construct(PollinationsAiUtility $utility, array $options)
    {
    ...
    public function get_utility(): PollinationsAiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PollinationsAiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PollinationsAiContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PollinationsAiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PollinationsAiConfig::make_feature($name);
    ...
    private static function buildUrl(PollinationsAiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PollinationsAiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PollinationsAiResponse($fetched);
        }
    ...
    private function fail(PollinationsAiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PollinationsAiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
