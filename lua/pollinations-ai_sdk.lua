...
local PollinationsAiSDK = {}
PollinationsAiSDK.__index = PollinationsAiSDK
...
PollinationsAiSDK._make_feature = _make_feature


function PollinationsAiSDK.new(options)
  local self = setmetatable({}, PollinationsAiSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function PollinationsAiSDK:options_map()
...
function PollinationsAiSDK:get_utility()
  return Utility.copy(self._utility)
end


function PollinationsAiSDK:get_root_ctx()
  return self._rootctx
end


function PollinationsAiSDK:prepare(fetchargs)
...
function PollinationsAiSDK:direct(fetchargs)
...
-- by passing it.
function PollinationsAiSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function PollinationsAiSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "PollinationsAiSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function PollinationsAiSDK:GenerateText(data)
  ...
  if data == nil then
    if self._generate_text == nil then
      self._generate_text = EntityMod.new(self, nil)
    end
...
function PollinationsAiSDK:ImageGeneration(data)
  ...
  if data == nil then
    if self._image_generation == nil then
      self._image_generation = EntityMod.new(self, nil)
    end
...
function PollinationsAiSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = PollinationsAiSDK.new(sdkopts)
  sdk.mode = "test"
