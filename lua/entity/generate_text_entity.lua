...
local GenerateTextEntity = {}
GenerateTextEntity.__index = GenerateTextEntity


function GenerateTextEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GenerateTextEntity)
  self._name = "generate_text"
  self._client = client
...
function GenerateTextEntity:get_name()
  return self._name
end


function GenerateTextEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GenerateTextEntity.new(self._client, opts)
end
...
function GenerateTextEntity:mark_deleted()
  self._deleted = true
end


function GenerateTextEntity:deleted()
  return true == self._deleted
end


function GenerateTextEntity:data_set(args)
  if args ~= nil then
...
function GenerateTextEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GenerateTextEntity:match_set(args)
  if args ~= nil then
...
function GenerateTextEntity:match_get()
...
function GenerateTextEntity:stream(action, args, callopts)
...
---@param reqdata GenerateTextCreateData
---@param ctrl? table
---@return GenerateText
---@return string? err
function GenerateTextEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function GenerateTextEntity:_run_op(ctx, post_done)
  local utility = self._utility
