...
The Lua SDK for the PollinationsAi API. Provides an entity-oriented interface using Lua conventions.
...
```lua
-- Create
local created, _ = client:GenerateText(nil):create({ name = "Example" }, nil)
...
local result, err = client:PollinationsAi(nil):load(
  { id = "test01" }, nil
)
