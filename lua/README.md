# PollinationsAi Lua SDK



The Lua SDK for the PollinationsAi API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:GenerateText()` — each with the same small set of operations (`load`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
| `messages` | `table` | Array of message objects for the conversation |
...
```
lua/
├── pollinations-ai_sdk.lua    -- Main SDK module
...
The main module (`pollinations-ai_sdk`) exports the SDK constructor
