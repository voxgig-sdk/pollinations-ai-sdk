...
[MCP](https://modelcontextprotocol.io) server exposing the PollinationsAi SDK as
two agent tools — `pollinations-ai_list` and `pollinations-ai_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pollinations-ai-mcp)
make build

# 2. Provide credentials via the environment
export POLLINATIONS_AI_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user pollinations-ai \
  -- /absolute/path/to/pollinations-ai-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./pollinations-ai-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// pollinations-ai_list: first page of records
{ "entity": "generate_text" }
{ "entity": "generate_text", "query": { } }

// pollinations-ai_load: one record by id
{ "entity": "image_generation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pollinations-ai-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user pollinations-ai \
     -- "$PWD"/dist/*/pollinations-ai-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `pollinations-ai_list` and `pollinations-ai_load` tools now appear
   in new sessions. Ask the agent to *"list generate_text using pollinations-ai"*
   and it calls `pollinations-ai_list` with `{"entity":"generate_text"}`.
...
```sh
export POLLINATIONS_AI_APIKEY=sk_live_xxx            # API key
export POLLINATIONS_AI_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `pollinations-ai_list` tool
...
### Call the `pollinations-ai_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `pollinations-ai_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `pollinations-ai_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `POLLINATIONS_AI_APIKEY` | API key sent with every request. |
| `POLLINATIONS_AI_BASE` | Optional override of the API base URL. |
...
```sh
./pollinations-ai-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"pollinations-ai_load","arguments":{"entity":"image_generation","query":{"id":1}}}}'
```
