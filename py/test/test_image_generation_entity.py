...
class TestImageGenerationEntity:

    def test_should_create_instance(self):
        testsdk = PollinationsAiSDK.test(None, None)
        ent = testsdk.ImageGeneration(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _image_generation_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "image_generation." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set POLLINATIONS_AI_TEST_IMAGE_GENERATION_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        image_generation_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.image_generation")))
        image_generation_ref01_data = None
        if len(image_generation_ref01_data_raw) > 0:
...
def _image_generation_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/image_generation/ImageGenerationTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("POLLINATIONS_AI_TEST_IMAGE_GENERATION_ENTID"))
