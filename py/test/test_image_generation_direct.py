...
class TestImageGenerationDirect:

    def test_should_direct_load_image_generation(self):
        setup = _image_generation_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-image_generation", "live" if setup["live"] else "unit")
        if _skip:
...
def _image_generation_direct_setup(mockres):
