# PollinationsAi SDK test runner
...
class PollinationsAiTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PollinationsAiTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PollinationsAiTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PollinationsAiTestRunner.getenv("POLLINATIONS_AI_TEST_LIVE")
        override = PollinationsAiTestRunner.getenv("POLLINATIONS_AI_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PollinationsAiTestRunner.getenv(key)
        ...
        explain = PollinationsAiTestRunner.getenv("POLLINATIONS_AI_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if PollinationsAiTestRunner._test_control is not None:
            return PollinationsAiTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PollinationsAiTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PollinationsAiTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PollinationsAiTestRunner.load_test_control()
...
def load_env_local():
    PollinationsAiTestRunner.load_env_local()


def env_override(m):
    return PollinationsAiTestRunner.env_override(m)


def entity_data(v):
    return PollinationsAiTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PollinationsAiTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PollinationsAiTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PollinationsAiTestRunner.load_test_control()


def live_delay_ms():
    return PollinationsAiTestRunner.live_delay_ms()
