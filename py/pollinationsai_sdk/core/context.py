...
from pollinationsai_sdk.core.helpers import get_ctx_prop, to_map
...
class PollinationsAiContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = PollinationsAiControl()
        ...
        if isinstance(sp, PollinationsAiSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, PollinationsAiResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, PollinationsAiResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return PollinationsAiOperation({})
    ...
    def make_error(self, code, msg):
        return PollinationsAiError(code, msg, self)
