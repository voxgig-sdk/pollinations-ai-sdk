...
boru-driven command-line client **and** interactive REPL for the PollinationsAi
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pollinations-ai-cli)
make build

# 2. See usage (words, entities, env vars)
./pollinations-ai-cli --help
...
POLLINATIONS_AI_BASE=https://api.example.com ./pollinations-ai-cli --help

# 6. No arguments -> interactive REPL
./pollinations-ai-cli
pollinations-ai> /help
pollinations-ai> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pollinations-ai-cli
   ```
...
Run with no arguments to open a REPL (prompt `pollinations-ai>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `generate_text`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/pollinations-ai-cli`. |
...
(`./pollinations-ai-cli <expr>`) and interactively (the REPL), and expressions compose the
