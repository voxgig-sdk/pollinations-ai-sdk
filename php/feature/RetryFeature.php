...
// PollinationsAi SDK retry feature
...
class PollinationsAiRetryFeature extends PollinationsAiBaseFeature
{
    ...
    public function init(PollinationsAiContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (PollinationsAiContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(PollinationsAiContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(PollinationsAiContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
