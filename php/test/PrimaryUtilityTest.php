...
// PollinationsAi SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        PollinationsAiSDK $client,
        PollinationsAiUtility $utility
    ): PollinationsAiContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new PollinationsAiContext($ctxmap, null);
    ...
    private static function fixctx(PollinationsAiContext $ctx, PollinationsAiSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?PollinationsAiError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new PollinationsAiError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new PollinationsAiResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new PollinationsAiBaseFeature();
    ...
    {
        $hook_client = PollinationsAiSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends PollinationsAiBaseFeature {
            public $hook_fn = null;
            public function TestHook(PollinationsAiContext $ctx): void
            {
    ...
    {
        $init_client = PollinationsAiSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends PollinationsAiBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new PollinationsAiSDK([
            'system' => [
    ...
    {
        $blocked_client = new PollinationsAiSDK([
            'system' => [
    ...
    public function test_make_fetch_def_basic(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new PollinationsAiSpec([
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new PollinationsAiSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = PollinationsAiSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new PollinationsAiOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = PollinationsAiSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof PollinationsAiError) ? $result->err->msg : (string)$result->err;
