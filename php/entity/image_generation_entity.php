...
// PollinationsAi SDK ImageGeneration entity
...
class ImageGenerationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ImageGenerationEntity($this->_client, $opts);
    }

    /**
     * @param ImageGeneration|array $args ImageGeneration data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = PollinationsAiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ImageGeneration|array The current ImageGeneration data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ImageGeneration fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = PollinationsAiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ImageGeneration fields).
     */
     ...
     *
     * @param ImageGenerationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ImageGenerationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ImageGeneration|array The loaded ImageGeneration as an assoc-array at the
     *   SDK boundary; throws PollinationsAiError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = PollinationsAiHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
